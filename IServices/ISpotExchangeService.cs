...
namespace FuturesBot.Services
{
    ...
    ///
    /// FIXES (important):
    /// 1) Holding calc: use Free + Locked (SpotHolding) so OMS doesn't get confused when OCO locks coins.
    /// 2) Maker miss fill: reprice loop + MARKET fallback using PlaceMarketBuyByQuoteAsync after N reprices.
    /// 3) Exit: cancel orders first, then refresh holding before selling (to release locked qty).
    /// 4) Safer open-orders handling (null-safe).
    /// </summary>
    public sealed class SpotOrderManagerService
    {
        ...
        // prevent infinite maker reprice loop
        private readonly ConcurrentDictionary<string, int> _entryRepriceCount = new();
        private const int MaxRepriceBeforeMarketFallback = 2; // after 2 reprices => market fallback
        ...
        public async Task TickAsync(TradeSignal? signal, CoinInfo coinInfo, CancellationToken ct)
        {
            ...
            // ------------------------------------------------------------
            // 1) Rescue OCO if holding but no open orders
            // ------------------------------------------------------------
            if (inPosition)
            {
                ...
                if (open == null || open.Count == 0)
                {
                    ...
                    // In this branch we have no open orders, so Free is safe to use for OCO qty.
            ...
            if (signal.Type == SignalType.Short)
            {
                ...
                // Refresh holding after cancel (release locked qty)
            ...
            {
                // meaningful holding => no entry
                if (inPosition) return;

                // dust does NOT block entry (prevents "stuck forever" on tiny BTC)
                if (IsThrottled(symbol)) return;

                // ================================
                // Sizing:
                // - Total Spot cap: Spot.WalletCapUsd
                ...
                // Apply quote buffer (use SpotOms config directly)
                ...
                // Maker-only entry: if pending BUY LIMIT exists -> wait until EntryRepriceSeconds, else reprice.
                ...
                if (pendingBuy != null)
                {
                    ...
                    // After too many reprices, do MARKET BUY by quote to ensure fill.
                    if (count > MaxRepriceBeforeMarketFallback)
                    {
                        var market = await _spot.PlaceMarketBuyByQuoteAsync(symbol, usdToUse);

                        await _notify.SendAsync(
                            $"[SPOT][OMS] MARKET fallback BUY {symbol} after {count - 1} reprices. " +
                            $"spend≈{usdToUse:0.##} {quoteAsset}, lastPrice={price:0.##}, orderId={market.OrderId}, dust={hasDust}. (reason={signal.Reason})");
                ...
                {
                    // PlaceLimitBuyAsync should validate rules and return REJECTED when too small.
                    return;
                }
        ...
        {
            // SpotHolding in your project: Free + Locked
            // If Locked is 0 or not used, still safe.
            return (h?.Free ?? 0m) + (h?.Locked ?? 0m);
        }
